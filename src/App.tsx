  ...
  return (
    <main className="shell">
      <section className="hero">
        <h1 className="eyebrow">Laboratorio 1 · Despliegue con GitHub</h1>
        <h2 className="display-title">Release Board V1</h2>
      ...
      <section className="grid">
        <article className="card card-accent">
          <h3>Entorno actual</h3>
          <p className="badge">{environment}</p>
          <p>Este valor cambia en cada build y nos ayuda a verificar que entorno estamos viendo.</p>
        </article>

        <article className="card">
          <h3>Version visible</h3>
          <p className="mono">{version}</p>
          <p>Usaremos el SHA corto del commit para identificar que version llego a staging.</p>
        </article>

        <article className="card">
          <h3>Que estamos practicando</h3>
          <ul>
            {notes.map((item) => (
              <li key={item}>{item}</li>
            ))}
